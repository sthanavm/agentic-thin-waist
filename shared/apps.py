...
REGISTRY: dict[str, AppSpec] = {
    ...
    "vimeo": AppSpec(
        name="vimeo",
        kind=HTML5_VIDEO,
        default_url="https://player.vimeo.com/video/911411289?autoplay=1&muted=1",
        bitrate_source="none",
        domains=("vimeocdn.com", "vimeo.com"),
        color="#1AB7EA",
        display_name="Vimeo",
        notes=(
            "Generic <video>; no vendor stats API, so bitrate is null unless "
            "byte-derived. "
            + "Default is a ~30 min clip on purpose: the previous 62 s default ended mid-run for any duration_s >= 60, which capped watched-time and hid real contention effects behind end-of-video."
        ),
    ),
    "tubi": AppSpec(
        name="tubi",
    ...
    "twitch": AppSpec(
        ...
        default_url="https://www.twitch.tv/relaxbeats",
        live=True,
        bitrate_source="none",
        domains=(
            "twitch.tv",
