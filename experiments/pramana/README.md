...
Every run should be checked before it is used as a result, against the rules the
project holds itself to:

**Hard rules** — a failure here is a contradiction inside the run: shaping stayed
under cap, label-vs-player honesty (no run claims an app streamed without player
evidence), no "played but watched zero", correct `no_playback` detection, bitrate
sanity, promised artifacts present, capture duration not short.

**Soft rules** — plausibility, not proof: resolution falling as bandwidth rises,
record bytes diverging from the pcap, duplicate configs.

Each run keeps its own `capture.pcap`, so any claim in `record.json` can be
re-derived independently from the packets — that cross-check is the ultimate
arbiter and needs nothing but the capture and `tcpdump`.
