...
_DEFAULT = (
    Path(
        os.environ.get(
            "PRAMANA_RESULTS_DIR", str(Path(__file__).resolve().parent / "results")
        )
    )
    / "pramana_runs"
)
            ...
            rows.append(
                {
                    ...
                    "resolutions_observed": "|".join(
                        map(str, q.get("resolutions_observed") or [])
                    )
                    or None,
                    ...
                    "connection_speed_estimate_mbps": q.get(
                        "connection_speed_estimate_mbps"
                    ),
...
if __name__ == "__main__":
    ...
    print(
        f"{len(rows)} app-rows across {len(compact)} runs "
